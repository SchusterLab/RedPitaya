...
class Decimation(UnsignedInteger):
    ...
    def to_binary(self, val):
        assert val in [1, 8, 64, 1024, 8192, 65536], "Value is  {val}. It should be in 1,8, 64,1024,8192,65536".format(
            val=val)
...
class Scope2(MemoryInterface):
    ...
    def acquire_avg(self, samples, avgs=1, trigger_source=TriggerSource.immediately):
        self.reset_writestate_machine(v=True)
        ...
        return self.rawdata_ch1(samples) / max(1, self.avg_cnt), self.rawdata_ch2(samples) / max(1, self.avg_cnt)
